...
const conversionAmmount = eventDataOverride.amount || eventModel.value || '';

const conversionValue =  JSON.parse(data.conversionValue) || {"currencyCode": conversionCurrency, "amount": conversionAmmount.toString()};

conversion_event.userData.userInfo = getUserInfo();
conversion_event.userData.userIds = getUserIds();
...
function getFirstPartyAdsTrackingUuid() {
  ...
  const eventLinkedinFirstPartyID = (eventModel.user_data != null && eventModel.user_data.linkedinFirstPartyId != null) ? eventModel.user_data.linkedinFirstPartyId : '';
