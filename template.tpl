...
[
  {
    "type": "TEXT",
    "name": "apiAccessToken",
    "displayName": "CAPI GTM Access Token, generated via LinkedIn Campaign Manager *Required*",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "apiVersion",
    "displayName": "API version, e.g. 202402 - should not be more than 12 months from current data *Required*",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "conversionRuleUrn",
    "displayName": "CAPI Conversion Rule ID defined in LinkedIn Campaign Manager *Required*",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
...
// Define LinkedIn Conversion API endpoint
...
// Determine conversion rule ID and URN
...
// if userLinkedinFirstPartyID is not set in eventModel.user_data, we try to find it in user properties
if (userLinkedinFirstPartyID == '') {
...
conversion_event.userData.userIds = [];

// Add hashed email if exists
if (userEmailSHA256 != '') {
  conversion_event.userData.userIds.push({ 'idType': 'SHA256_EMAIL', 'idValue': userEmailSHA256 });
}

// Add LinkedinFirstPartyID if exists
if (userLinkedinFirstPartyID != '') {
  conversion_event.userData.userIds.push({ 'idType': 'LINKEDIN_FIRST_PARTY_ADS_TRACKING_UUID', 'idValue': userLinkedinFirstPartyID });
}
...
// Prepare user information for conversion event
if (userFirstName !== '' || userLastName !== '' || userCompanyName !== '' || userJobTitle !== '' || userCountryCode !== '') {
  conversion_event.userData.userInfo = {};

  if (userFirstName !== '') {
    conversion_event.userData.userInfo.firstName = userFirstName;
  }
  if (userLastName !== '') {
    conversion_event.userData.userInfo.lastName = userLastName;
  }
  if (userCompanyName !== '') {
    conversion_event.userData.userInfo.companyName = userCompanyName;
  }
  if (userJobTitle !== '') {
    conversion_event.userData.userInfo.title = userJobTitle;
  }
  if (userCountryCode !== '') {
    conversion_event.userData.userInfo.countryCode = userCountryCode;
  }
}
...
// Prepare POST body for API call
var postBody = {
...
function validateUserData() {
  ...
  if (data_is_valid_flag == false) {
    if (conversion_event.userData.userInfo != null && conversion_event.userData.userInfo.firstName != "" && conversion_event.userData.userInfo.lastName != "") {
      data_is_valid_flag = true;
    }
  }
...
// Function to send conversion event to LinkedIn API
...
Created on 3/14/2024, 4:29:13 PM
